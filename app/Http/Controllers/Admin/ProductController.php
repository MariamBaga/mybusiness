...
class ProductController extends Controller
{
    ...
    {
        $products = Product::with(['partner', 'category'])
            ->latest()
            ->paginate(10);
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('specifications') && isset($request->specifications['key']) && isset($request->specifications['value'])) {
            $specifications = [];
   ...
   public function update(Request $request, Product $product)
{
    ...
    \Log::info('Product update request:', [
        ...
        'has_is_sponsored' => $request->has('is_sponsored'),
        'status_value' => $request->input('status'),
        'is_featured_value' => $request->input('is_featured'),
        'is_sponsored_value' => $request->input('is_sponsored'),
    ]);

    $data = $request->except(['image', 'specifications', '_token', '_method']);

    // CORRECTION CRITIQUE : Traitement correct des checkboxes
    $data['status'] = $request->has('status') ? 1 : 0;
    $data['is_featured'] = $request->has('is_featured') ? 1 : 0;
    $data['is_sponsored'] = $request->has('is_sponsored') ? 1 : 0;
    ...
    \Log::info('Product after update:', [
        ...
        'is_sponsored' => $product->fresh()->is_sponsored,
    ]);
